...
defineProperty(promise.unresolved, 'invoke', d(function (name) {
	var def;
	if (!this.pending) {
		this.pending = [];
	}
...
promise.onswitch.invoke = function (args, resolve) {
	var fn;
	if (this.failed) {
		resolve(this);
		return;
...
defineProperty(promise.resolved, 'invoke', d(function (fn) {
	var args, def;
	if (this.failed) {
		return this;
	}
